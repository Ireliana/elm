...
import { saveToLocal, loadFromLocal } from '../../public/js/store';
...
export default {
  ...
  methods: {
    initScroll() {
      if (!this.sellerScorll) {
        this.sellerScorll = new BScroll(this.$refs.seller, {
          click: true,
        });
      }
      this.sellerScorll.refresh(); 
    },
    initPicScroll() {
      if (this.seller.pics) {
        let width = 120;
        let margin = 6;
        let picWidth = (width * this.seller.pics.length) - margin;
